...
public class UiChecks extends Instrumentation {
    ...
    @Override public void onStart(){Bundle result=new Bundle();try{
        ...
        screen("Settings");screen("Appearance");settle();check(find(visibleRoot(),"← Back")==null,"Duplicate boxed Back button remains");screenshot("08-appearance-single-back");invoke("navigateBack",new Class[]{});check(field("tab").equals("Settings"),"Appearance back failed");
        screen("Library");screen("Settings");invoke("navigateBack",new Class[]{});check(field("tab").equals("Library"),"Back lost previous section");
        JSONObject office=Store.object("id","office-route","name","Lecture.pptx","file","office-fixture","chapter","chapter","type","application/vnd.openxmlformats-officedocument.presentationml.presentation");
        // An unregistered note must be rejected; then seed the same stored attachment a real import creates.
        invoke("readFile",new Class[]{JSONObject.class},office);check(!((Boolean)field("reading")),"Deleted note guard failed");
        try(java.util.zip.ZipOutputStream zip=new java.util.zip.ZipOutputStream(getTargetContext().openFileOutput("office-fixture",Context.MODE_PRIVATE))){
            zip.putNextEntry(new java.util.zip.ZipEntry("ppt/slides/slide1.xml"));
            zip.write("<p:sld xmlns:p=\"http://schemas.openxmlformats.org/presentationml/2006/main\"><p:cSld><p:spTree/></p:cSld></p:sld>".getBytes(java.nio.charset.StandardCharsets.UTF_8));zip.closeEntry();
        }
        runOnMainSync(()->{try{Store store=(Store)field("store");store.array("notes").put(office);store.save();}catch(Exception e){throw new RuntimeException(e);}});
